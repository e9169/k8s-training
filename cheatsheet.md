...
kubectl run nginx-deployment --image=nginx:1.14.2 --restart Never   # Create plain pod
kubectl create deployment nginx --image=nginx:1.14.2      # Create deployment

kubectl set image deployment/nginx nginx=nginx:1.16.1    # Update image in deployment pod template
